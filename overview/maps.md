...
### 1. Shell State Assumption
```
# WRONG
shell("cd /tmp")
shell("ls")  # CWD reset, ls runs in random directory

# CORRECT
shell("cd /tmp && ls")  # Chain in single call
```

### 2. Validation Skipping
```
# WRONG (XLSX)
ipython: Create workbook
ipython: Save
# Skip validation
KIMI_REF output.xlsx  # May have formula errors

# CORRECT
ipython: Save
shell: recheck → Must pass
shell: reference-check → Must pass
shell: validate → Must pass
KIMI_REF output.xlsx
```

### 3. PivotTable Order Violation
```
# WRONG
shell: KimiXlsx pivot input.xlsx temp.xlsx
ipython: wb = openpyxl.load_workbook('temp.xlsx')
ipython: Add cover sheet
ipython: Save
# File corrupted!

# CORRECT
ipython: Create ALL sheets (including cover)
ipython: Save as base.xlsx
shell: KimiXlsx pivot base.xlsx final.xlsx
# Never modify final.xlsx again
```

### 4. Browser State Confusion
```
# WRONG
browser_visit(url="site.com")
browser_click(element_index=2)  # Which citation_id?

# CORRECT
browser_visit(url="site.com")  # Returns citation_id=1
browser_click(citation_id=1, element_index=2)
```

## Step Budget Optimization

### Base Chat (10 steps/turn)
| Workflow | Tools Needed | Steps |
|----------|--------------|-------|
| Simple search | web_search | 1 |
| Data analysis | web_search + ipython + write_file | 3 |
| Image search | search_image_by_text + ipython | 2 |

### OK Computer (200-300 steps/session)
| Workflow | Tools Needed | Steps |
|----------|--------------|-------|
| DOCX document | read_file×2 + shell + ipython + write_file + read_file | 6 |
| XLSX 3-sheet | read_file + ipython×3 + shell×9 + KIMI_REF | 13 |
| PDF (HTML) | read_file×2 + ipython + write_file + shell + KIMI_REF | 6 |
| WebApp | read_file + shell×3 + ipython + write_file×N + deploy | 10+N |

## Critical Path Analysis

### DOCX Critical Path
1. Read SKILL.md (~200ms)
2. Read Example.cs (~100ms)
3. Generate Program.cs (~500ms)
4. dotnet build (~2000ms) ← **DOMINANT**
5. Validation (~300ms)

**Optimization**: Minimize compile errors to avoid rebuilds.

### XLSX Critical Path
1. Read SKILL.md (~200ms)
2. Per-sheet creation (~1000ms/sheet)
3. Per-validation (~500ms/sheet)
4. Final validation (~500ms)

**Optimization**: Validate early and often to catch errors fast.

### PDF Critical Path
1. Read SKILL.md + route docs (~300ms)
2. HTML generation (~500ms)
3. Playwright render (~3000ms) ← **DOMINANT**

**Optimization**: Choose LaTeX route for math-heavy content (faster than HTML+KaTeX).

### WebApp Critical Path
1. Read SKILL.md (~200ms)
2. npm install (~5000ms) ← **DOMINANT** (cached after first)
3. Build (~5000ms)
4. Deploy (~2000ms)

**Optimization**: Template reuse minimizes npm install calls.
